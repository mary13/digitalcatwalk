...
void JSdraw() {
  ...
  for (int i = 0; i < cols;i++) {
    ...
    for (int j = 0; j < rows;j++) {
      ...
      int loc = (trailsImg.width + x/display_multiplier - 10) + y/display_multiplier*trailsImg.width; // x to mirror the image
  ...
  renderTexture(pgl.gl);    ///// FOR SYPHO
}
