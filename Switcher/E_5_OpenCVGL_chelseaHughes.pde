  ...
  for (int i = 0; i < cols;i++) {
    ...
    for (int j = 0; j < rows;j++) {
      ...
      if (sz > 9) {
        ...
        fill(255, 188, 0, random(100));


        beginShape();
        vertex(x + cellSize/2-20, y + cellSize/2);
        vertex(x + cellSize/2 +20, y + cellSize/2+10);
        vertex(x + cellSize/2+40, y + cellSize/2+50);
        vertex(x + cellSize/2+20, y + cellSize/2+20);
        vertex(x + cellSize/2, y + cellSize/2+50);
        endShape(CLOSE);
      }
      else if (sz <= 9) {
        noStroke();
        fill(0, 232, 255, random(255));
