...
void BWdraw() {
  ...
  for (int i = 0; i < cols;i++) {
    ...
    for (int j = 0; j < rows;j++) {
      ...
      if (sz > 9) {
        stroke(157, 255, 236, random(255));
        strokeWeight(random(3));
        line((x + cellSize/2)+30, (y + cellSize/2)+15, ((x + cellSize/2)+30), ((y + cellSize/2)-15));

        stroke(26, 255, 239, random(255));
        strokeWeight(random(3));
        line((x + cellSize/2)-30, (y + cellSize/2), ((x + cellSize/2)+60), ((y + cellSize/2)));


        noFill();

        fill(236, 255, 134, random(255));
        noStroke();
        rect(x + cellSize/2, y + cellSize/2, sz+5, sz+5);
      }
      else if (sz <= 9) {

        fill(253, 255, 240, random(255));
